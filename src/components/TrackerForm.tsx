            ...
            >
                <div className="p-4 border-b border-app-border flex justify-between items-center bg-app-bg/50">
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="p-1 rounded-full hover:bg-app-bg text-app-text-muted"
                    >
                        <X size={20} />
                    </button>
                </div>
                    ...
                    <div className="pt-2 flex justify-between">
                        {isCurrentlyRecurring ? (
                            <button
                                ...
                                className="px-4 py-2 rounded-lg text-sm font-medium text-app-accent-error hover:bg-app-accent-error/10 transition-colors"
                            >
                                Stop Repeating
                            </button>
                        ) : (
...
const TrackerForm = memo(({ subjects, setSubjects }: TrackerFormProps) => {
    const [editingRecurringIndex, setEditingRecurringIndex] = useState<number | null>(null);
    ...
    const handleRecurringClick = useCallback((index: number) => {
            ...
            <AnimatePresence>
                {editingRecurringIndex !== null && (
                    <RecurringModal
                        ...
                        isCurrentlyRecurring={subjects[editingRecurringIndex]?.recurring || false}
                    />
                )}
            </AnimatePresence>
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <div className="flex-1 flex items-center gap-1.5">
                                <span className="text-[10px] uppercase text-app-text-muted font-medium shrink-0">
                                    Plan
                                </span>
                                <input
                                    type="number"
                ...
                <table className="w-full text-left text-sm text-app-text-muted min-w-[600px]">
                    <thead className="bg-app-bg/50 text-xs uppercase text-app-text-main">
                        <tr>
                            ...
                            <th className="px-4 md:px-6 py-3 w-10" title="Recurring Days">
                                Repeat
                            </th>
                            ...
                            >
                                ...
                                <td className="px-4 md:px-6 py-3 text-center">
                                    <button
                                        type="button"
                                        onClick={() => handleRecurringClick(index)}
                                        className={`flex items-center justify-center gap-1 p-1.5 rounded-full transition-colors ${
                                            subject.recurring
                                                ? 'bg-app-primary text-white hover:bg-app-primary-hover'
                                                : 'text-app-text-muted hover:bg-app-bg hover:text-app-primary'
                                        }`}
                                    ...
                                    >
                                        <Repeat2 size={16} />
                                        {subject.recurring &&
                                            subject.recurringDays &&
                                            subject.recurringDays.length < 7 && (
                                                <span className="text-[10px] font-bold">
                                                    {subject.recurringDays.length}
                                                </span>
                                            )}
                                    </button>
                                </td>
