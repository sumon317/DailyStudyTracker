...
const QualityCheckItem = memo(({ check, onToggle, onUpdateLabel, onRemove }: QualityCheckItemCompProps) => (
...
const RatingOption = memo(({ option, isSelected, onSelect }: RatingOptionProps) => (
    <label
        className={`flex flex-1 cursor-pointer flex-col items-center justify-center rounded-lg border p-2 sm:p-3 text-xs sm:text-sm font-medium transition-all ${
            isSelected
...
const QualityCheck = memo(({ checks, setChecks, rating, setRating }: QualityCheckProps) => {
    ...
    const addItem = useCallback(() => {
        setChecks((prev) => [...prev, { id: Date.now(), label: '', checked: false }]);
    }, [setChecks]);
